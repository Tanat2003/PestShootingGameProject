...
public class Weapon
{
    ...
    private int defaultAmmoInMagazine;
    private int defaultBulletDamage;
    private int defaultMagazineCapacity;

    [Header("Weapon Upgrade Detail")]
    public int maxUpgradeWeapon = 4;
    public int currentUpgradeWeapon ;
    ...
    public Weapon(WeaponData weaponData) //constructorของข้อมูลปืน
    {
        ...
        unlockedWeapon = weaponData.unlockedWeapon;
        ...
        currentUpgradeWeapon = weaponData.currentUpgradeWeapon;

        defaultAmmoInMagazine =weaponData.defaultAmmoInMagazine;
        defaultBulletDamage = weaponData.defaultBulletDamage;
        defaultMagazineCapacity = weaponData.defaultMagazineCapacity;
    ...
    #endregion
    public void UpgradeAttackWeapon()
    {

        currentUpgradeWeapon++;
        if (currentUpgradeWeapon >= maxUpgradeWeapon)
        {
            currentUpgradeWeapon = maxUpgradeWeapon;
            weaponData.currentUpgradeWeapon = currentUpgradeWeapon;
            return;
        }
        bulletDamage += Mathf.RoundToInt(bulletDamage * 0.5f);
        weaponData.bulletDamage = bulletDamage;
        weaponData.currentUpgradeWeapon = currentUpgradeWeapon;
    ...
    public void UpgradeMagazineCapacity()
    {
        ...
        ammoInMagazine += Mathf.RoundToInt(ammoInMagazine * 0.25f);
        magazineCapacity += Mathf.RoundToInt(magazineCapacity * 0.25f);
        weaponData.magazineCapacity = magazineCapacity;
        weaponData.ammoInMagazine = ammoInMagazine;
        weaponData.currentUpgradeWeapon = currentUpgradeWeapon;


        
    }
    public void ResetCurrentUpgradeWeapon()
    {
        currentUpgradeWeapon = 0;
        ammoInMagazine = defaultAmmoInMagazine;
        magazineCapacity = defaultMagazineCapacity;
        bulletDamage = defaultBulletDamage;

        weaponData.defaultBulletDamage = defaultBulletDamage;
        weaponData.defaultAmmoInMagazine= defaultAmmoInMagazine;
        weaponData.defaultMagazineCapacity= defaultMagazineCapacity;
